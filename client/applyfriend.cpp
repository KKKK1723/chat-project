    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    qDebug() << "parent is:" << (parent ? "valid" : "null");
    ...
    // 检查 UI 控件是否有效
    if (!ui->name_ed || !ui->lb_ed || !ui->back_ed || !ui->input_tip_wid) {
        qDebug() << "ERROR: UI controls are null!";
        qDebug() << "name_ed:" << (ui->name_ed ? "valid" : "null");
        qDebug() << "lb_ed:" << (ui->lb_ed ? "valid" : "null");
        qDebug() << "back_ed:" << (ui->back_ed ? "valid" : "null");
        qDebug() << "input_tip_wid:" << (ui->input_tip_wid ? "valid" : "null");
        return;
    }
    
    qDebug() << "Setting placeholder texts...";
    ui->name_ed->setPlaceholderText(tr("布布"));
    ui->lb_ed->setPlaceholderText("搜索、添加标签");
    ui->back_ed->setPlaceholderText("燃烧的胸毛");
    qDebug() << "Placeholder texts set successfully";

    qDebug() << "Setting lb_ed properties...";
    ui->lb_ed->SetMaxLength(21);
    ui->lb_ed->move(2, 2);
    ui->lb_ed->setFixedHeight(20);
    ui->lb_ed->SetMaxLength(10);
    qDebug() << "lb_ed properties set successfully";
    
    qDebug() << "Hiding input_tip_wid...";
    ui->input_tip_wid->hide();
    qDebug() << "input_tip_wid hidden successfully";

    _tip_cur_point = QPoint(5, 5);

    _tip_data = { "同学","家人","算法比赛","足球","老师",
                 "资料","nodejs开发指南","go 语言开发指南",
    ...
    qDebug() << "=== ApplyFriend constructor completed ===";
}
...
{
    qDebug() << "=== SetSearchInfo called ===";
    qDebug() << "si is:" << (si ? "valid" : "null");
    
    if (!si) {
        qDebug() << "ERROR: SearchInfo is null in SetSearchInfo!";
        return;
    }
    
    qDebug() << "Assigning SearchInfo to _si...";
    _si = si;
    qDebug() << "SearchInfo assigned to _si";
    
    // 检查 UI 控件是否有效
    qDebug() << "Checking UI validity...";
    qDebug() << "ui is:" << (ui ? "valid" : "null");
    if (!ui) {
        qDebug() << "ERROR: ui is null!";
        return;
    }
    
    qDebug() << "Checking individual UI controls...";
    qDebug() << "ui->name_ed is:" << (ui->name_ed ? "valid" : "null");
    qDebug() << "ui->back_ed is:" << (ui->back_ed ? "valid" : "null");
    
    if (!ui->name_ed || !ui->back_ed) {
        qDebug() << "ERROR: UI controls are null!";
        return;
    }
    
    qDebug() << "UI controls are valid, proceeding...";
    
    // 检查 UserMgr 是否有效
    qDebug() << "Getting UserMgr instance...";
    auto userMgr = UserMgr::GetInstance();
    qDebug() << "UserMgr is:" << (userMgr ? "valid" : "null");
    
    if (!userMgr) {
        qDebug() << "ERROR: UserMgr is null!";
        // 即使 UserMgr 无效，也设置 SearchInfo 的名称
        qDebug() << "Setting back_ed text to si->_name...";
        ui->back_ed->setText(si->_name);
        qDebug() << "Set back_ed to si->_name:" << si->_name;
        qDebug() << "=== SetSearchInfo completed (partial) ===";
        return;
    }
    
    qDebug() << "UserMgr is valid, getting user name...";
    auto applyname = userMgr->GetName();
    qDebug() << "applyname:" << applyname;
    
    qDebug() << "Getting bakname from si...";
    auto bakname = si->_name;
    qDebug() << "bakname:" << bakname;
    
    qDebug() << "Setting name_ed text...";
    ui->name_ed->setText(applyname);
    qDebug() << "name_ed text set successfully";
    
    qDebug() << "Setting back_ed text...";
    ui->back_ed->setText(bakname);
    qDebug() << "back_ed text set successfully";
    
    qDebug() << "=== SetSearchInfo completed ===";
}
